...
fn tuple_sum(a: &(usize, usize), b: &(usize, usize)) -> (usize, usize) {
    return (a.0 + b.0, a.1 + b.1);
}

struct TreapOrchestrator {
    aa: Vec<usize>,
    bb: Vec<usize>,
    k: usize,
    sum_treap: TreapList<(usize, usize)>,
}

impl TreapOrchestrator {
    fn new(ab: Vec<(usize, usize)>) -> Self {
        let aa: Vec<usize> = ab.iter().map(|(a, _)| *a).collect();
        let bb: Vec<usize> = ab.iter().map(|(_, b)| *b).collect();
        let k = 0;
        let sum_treap = TreapList::<(usize, usize)>::new(tuple_sum, (0, 0));
        let mut treap_orchestrator = TreapOrchestrator {
            aa,
            bb,
            k,
            sum_treap,
        ...
        for (i, (a, b)) in ab.iter().enumerate() {
            treap_orchestrator.insert(i, *a, *b);
        }
        return treap_orchestrator;
    }

    fn insert(&mut self, w: usize, x: usize, y: usize) {
        self.aa[w] = x;
        self.bb[w] = y;
        if y == 2 {
            self.k += 1;
        }
        self.sum_treap.insert((x, y));
    }

    fn remove(&mut self, index: usize) {
        let value = (self.aa[index], self.bb[index]);
        self.sum_treap.erase(value);
        if self.bb[index] == 2 {
            self.k -= 1;
        }
    }

    fn update(&mut self, w: usize, x: usize, y: usize) {
        self.remove(w);
        self.insert(w, x, y);
    }

    fn is_top_k_all_b2(&mut self) -> bool {
        // ここを高速に判定する
        // TopK の合計の B の値が 2 * K であれば、TopK は全て B=2 である
        let n: usize = self.aa.len();
        let k = self.k;
        let sum_y = self.sum_treap.prod(n - k, n).1;
        return sum_y == k * 2;
    }
    ...
    fn solve(&mut self) -> usize {
        let n = self.aa.len();
        if n == self.k {
            let double = self.sum_treap.prod(1, n).0;
            let single = self.sum_treap.prod(0, 1).0;
            return double * 2 + single;
        }
        if self.k == 0 {
            let single = self.sum_treap.prod(0, n).0;
            return single;
        }
        if self.is_top_k_all_b2() {
            let k = self.k;
            let hi_double = self.sum_treap.prod(n - k + 1, n).0;
            let hi_single = self.sum_treap.prod(n - k, n - k + 1).0;
            let m: usize = n - k;
            let lo_double = self.sum_treap.prod(m - 1, m).0;
            let lo_single = self.sum_treap.prod(0, m - 1).0;
            return hi_double * 2 + hi_single + lo_double * 2 + lo_single;
        } else {
            let k = self.k;
            let double = self.sum_treap.prod(n - k, n).0;
            let single = self.sum_treap.prod(0, n - k).0;
            return double * 2 + single;
        }
    }
}
...
fn main() {
    input! {
        ...
        q: usize,
        ab: [(usize, usize); n],
        wxy: [(usize, usize, usize); q],
    }
    let mut orc = TreapOrchestrator::new(ab);
    for (mut w, x, y) in wxy {
        w -= 1;
        orc.update(w, x, y);
        let ans = orc.solve();
        println!("{}", ans);
    ...
    pub fn erase(&mut self, val: V) -> bool {
        ...
        match m {
            ...
            Some(m_node) => {
                ...
                self.root = tl_merge(tl_merge(l, rest, self.op, &self.e), r, self.op, &self.e);
                true
            }
        }
    }
